# -*- coding: utf-8 -*-
"""
Created on Tue Jun 12 19:21:49 2018
...
ass_cand_name = []
for i in jk:
    ass_no = list(set(dataset["Party"][dataset["Name_of_Candidate"]==i]))
    if len(ass_no) > 1:
...
for i in ass_cand_name:
    elec_lst = list(set(dataset["Party"][dataset["Name_of_Candidate"]==i]))
    if len(elec_lst) > 1:
...
for c,f in zip(ppl_lst,freq):
    votes = []
    for const in f:
        votes.append(dataset["Votes"][(dataset["Party"]==const) & (dataset["Name_of_Candidate"]==c)].mean())
    total.append(votes)
