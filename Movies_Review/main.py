...
dataset2 = dataset.drop(['imdb_score','movie_title','movie_imdb_link','language','movie_facebook_likes'],axis = 1)

features = dataset2.iloc[:,0:].values
labels = dataset.iloc[:,17].values
...
cv = cross_val_score(estimator = ran_for_regressor,X = features_train,y = labels_train,cv = 10)
cv.mean()